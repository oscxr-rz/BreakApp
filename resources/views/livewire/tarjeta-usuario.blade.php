<div class="min-h-screen bg-gray-50" x-data="{
        ...
        <div class="max-w-md mx-auto pt-20 px-4">
            <div class="bg-white rounded-2xl shadow-sm p-8 text-center border border-gray-100">
                <div class="w-20 h-20 bg-[#FBE8DA] rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg class="w-10 h-10 text-[#951327]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    @elseif (!empty($tarjetas))
        ...
        <div class="max-w-2xl mx-auto px-4 py-6 pb-24">
            ...
            <button
                @click="mostrarModalAgregar = true; $nextTick(() => inicializarStripe())"
                ...
                <div class="w-11 h-11 rounded-full bg-[#FBE8DA] group-hover:bg-[#951327] flex items-center justify-center transition-colors">
                    <svg class="w-5 h-5 text-[#951327] group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
            ...
            <div class="space-y-4">
                @foreach ($tarjetas as $tarjeta)
                    <div class="group">
                        <!-- Tarjeta visual -->
                        <div class="relative w-full aspect-[1.75/1] rounded-2xl p-5 sm:p-6 text-white shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] {{ 
                            $tarjeta['marca'] === 'Visa' ? 'bg-linear-to-br from-[#1434CB] to-[#0B1F71]' :
                            ($tarjeta['marca'] === 'MasterCard' ? 'bg-linear-to-br from-[#0a0a0a] to-[#1a1a1a]' :
                            ($tarjeta['marca'] === 'American Express' ? 'bg-linear-to-br from-[#006FCF] to-[#00509E]' :
                            ($tarjeta['marca'] === 'Discover' ? 'bg-linear-to-br from-[#FF6000] to-[#E55400]' :
                            'bg-linear-to-br from-[#2D3748] to-[#1A202C]')))
                        }}">
                            
                            <!-- Patrón sutil de fondo -->
                            <div class="absolute inset-0 opacity-[0.03]">
                                <div class="absolute top-0 right-0 w-80 h-80 bg-white rounded-full -translate-y-1/3 translate-x-1/3"></div>
                            ...
                            <div class="relative h-full flex flex-col justify-between">
                                ...
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="flex items-center opacity-95">
                                        @if($tarjeta['marca'] === 'Visa')
                                            <img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" alt="Visa" class="h-4 sm:h-5 w-auto brightness-0 invert">
                                        @elseif($tarjeta['marca'] === 'MasterCard')
                                            <img src="https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg" alt="MasterCard" class="h-7 sm:h-9 w-auto">
                                        @elseif($tarjeta['marca'] === 'American Express')
                                            <img src="https://www.aexp-static.com/cdaas/one/statics/axp-static-assets/1.8.0/package/dist/img/logos/dls-logo-bluebox-solid.svg" alt="American Express" class="h-8 sm:h-10 w-auto">
                                        @elseif($tarjeta['marca'] === 'Discover')
                                            <img src="https://www.discover.com/content/dam/discover/en_us/credit-cards/card-acquisitions/grey-theme/images/cardart/cardart-secured-268x171.png" alt="Discover" class="h-6 sm:h-7 w-auto object-contain" style="filter: brightness(0) invert(1);">
                                        @else
                                            <svg viewBox="0 0 24 24" class="h-5 sm:h-6 w-auto" fill="none" stroke="currentColor" stroke-width="1.5">
                                    ...
                                    <!-- Chip EMV -->
                                    <div class="relative w-9 h-7 sm:w-11 sm:h-9 opacity-90">
                                        <div class="absolute inset-0 rounded-lg shadow-md bg-linear-to-br from-amber-200 via-amber-300 to-amber-400"></div>
                                        <div class="absolute inset-0.5 rounded-md bg-linear-to-tl from-amber-200 via-amber-300 to-amber-400"></div>
                                        <div class="absolute inset-[3px] sm:inset-1 grid grid-cols-3 grid-rows-2 gap-[1.5px] sm:gap-0.5 p-1">
                                ...
                                <!-- Número de tarjeta -->
                                <div class="my-auto">
                                    <p class="text-lg sm:text-xl md:text-2xl font-light tracking-[0.25em] sm:tracking-[0.3em] font-mono text-white/95">
                                ...
                                <div class="flex justify-between items-end gap-4">
                                    <div class="space-y-0.5 sm:space-y-1 min-w-0 flex-1">
                                        <p class="text-[8px] sm:text-[9px] font-medium tracking-wider uppercase opacity-60">Titular</p>
                                        <p class="text-xs sm:text-sm font-medium tracking-wide uppercase opacity-95 truncate">{{ session('nombre', 'Usuario') }}</p>
                                    </div>
                                    <div class="text-right space-y-0.5 sm:space-y-1 shrink-0">
                                        <p class="text-[8px] sm:text-[9px] font-medium tracking-wider uppercase opacity-60">Válida hasta</p>
                                        <p class="text-xs sm:text-sm font-medium tracking-wider opacity-95">
                        ...
                        <div class="mt-3">
                            <button @click="modalEliminar = true; tarjetaEliminar = {{ $tarjeta['id_tarjeta'] }}"
                                class="w-full py-3 sm:py-3.5 bg-white hover:bg-red-50 text-red-600 rounded-xl sm:rounded-2xl font-medium transition-all duration-200 flex items-center justify-center gap-2 border border-red-100 hover:border-red-200 shadow-sm hover:shadow">
    ...
    @else
        ...
        <div class="max-w-md mx-auto pt-20 px-4">
            <div class="bg-white rounded-2xl shadow-sm p-8 text-center border border-gray-100">
                ...
                <p class="text-gray-500 text-sm mb-6">Agrega una tarjeta para realizar pagos de forma segura y rápida</p>
                <button
                    @click="mostrarModalAgregar = true; $nextTick(() => inicializarStripe())"
    ...
    <div x-show="mostrarModalAgregar" x-transition:enter="transition ease-out duration-300"
        ...
        x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end justify-center"
        style="display: none;" @click.self="mostrarModalAgregar = false; errorPago = ''">
        ...
        <div x-show="modalEliminar" x-transition:enter="transition ease-out duration-400"
            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button @click="$wire.ocultarTarjeta(tarjetaEliminar); modalEliminar = false; tarjetaEliminar = null"
