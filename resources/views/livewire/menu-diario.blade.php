<div class="w-full px-4 md:px-6 lg:px-8 py-6">
    @if (!empty($menu))
        ...
        <header class="mb-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-2 lg:gap-3">
                    <div class="w-10 h-10 lg:w-12 lg:h-12 bg-slate-700 rounded-full flex items-center justify-center">
                        <img src="{{ asset('storage/logo.jpg') }}" alt="Logo">
                    </div>
                    <span class="font-semibold text-lg lg:text-xl">BreakApp</span>
                </div>

                <!-- Iconos Derecha -->
                <div class="flex items-center gap-2 lg:gap-3">
                    <a href="{{ route('carrito') }}"
                        class="relative w-10 h-10 lg:w-11 lg:h-11 flex items-center justify-center hover:bg-slate-100 rounded-full transition-colors">
                    ...
                    {{-- <a href="" class="w-10 h-10 lg:w-11 lg:h-11 flex items-center justify-center hover:bg-slate-100 rounded-full transition-colors">
                        <svg class="w-6 h-6 lg:w-7 lg:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Fecha -->
            <div class="flex items-center gap-2 text-sm lg:text-base text-gray-600 mb-4">
            ...
            <div class="relative">
                <input type="text" placeholder="Buscar productos..." id="buscarInput"
                    class="w-full bg-slate-100 rounded-xl lg:rounded-2xl px-4 py-3 lg:py-3.5 pl-11 lg:pl-12 text-sm lg:text-base border-none focus:ring-2 focus:ring-slate-300 transition-all">
                <svg class="w-5 h-5 lg:w-6 lg:h-6 absolute left-3 lg:left-4 top-1/2 -translate-y-1/2 text-gray-400"
        ...
        <section class="mb-6 lg:mb-8">
            <div
                ...
                <button data-categoria=""
                    class="boton-categoria shrink-0 px-4 py-2 rounded-full bg-slate-700 text-white text-sm lg:text-base font-medium transition-all">Todas</button>
                @foreach ($menu['productos'] as $categoria => $productos)
                    <button data-categoria="{{ strtolower($categoria) }}"
                        class="boton-categoria shrink-0 px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm lg:text-base font-medium transition-all">{{ $categoria }}</button>
                @endforeach
            </div>
        </section>
            ...
            @foreach ($menu['productos'] as $categoria => $productos)
                <section class="seccion-categoria mb-8 lg:mb-10" data-categoria="{{ strtolower($categoria) }}">
                    <div class="flex items-center justify-between mb-4 lg:mb-5">
                        <h2 class="font-bold text-lg lg:text-xl text-slate-800">{{ $categoria }}</h2>
                        <span class="text-sm lg:text-base text-slate-500">{{ count($productos) }} productos</span>
                    </div>

                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
                        @foreach ($productos as $producto)
                            <div class="tarjeta-producto bg-slate-50 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
                                ...
                                <div class="relative">
                                    ...
                                    {{-- <button class="absolute top-3 right-3 w-9 h-9 lg:w-10 lg:h-10 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform">
                                        <svg class="w-5 h-5 lg:w-6 lg:h-6 fill-none stroke-slate-700" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <div class="p-4 lg:p-5">
                                    <h3 class="font-bold text-base lg:text-lg mb-1 text-slate-800">
                                        {{ $producto['nombre'] }}</h3>
                                    <p class="text-xs lg:text-sm text-slate-600 mb-3 line-clamp-2">
                                        {{ $producto['descripcion'] }}</p>

                                    <!-- Tiempo -->
                                    <div class="flex items-center gap-1.5 mb-3 text-slate-500">
                                    ...
                                    <div class="flex items-center justify-between gap-3">
                                        <div>
                                            <p class="text-xs lg:text-sm text-slate-500">Precio</p>
                                            <p class="font-bold text-xl lg:text-2xl text-slate-800">
                                                ${{ number_format($producto['precio'], 2) }}</p>
                                        </div>

                                        <button wire:click="agregarAlCarrito({{ $producto['id_producto'] }}, 1)"
                                            wire:loading.attr="disabled"
                                            class="bg-amber-700 hover:bg-amber-800 disabled:bg-slate-400 text-white px-3 lg:px-4 py-2.5 lg:py-3 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 active:scale-95 text-sm lg:text-base shadow-md hover:shadow-lg"
        ...
        <div id="sinResultados" class="hidden flex-col items-center justify-center py-20 lg:py-32">
            <div class="w-32 h-32 lg:w-40 lg:h-40 bg-slate-100 rounded-full flex items-center justify-center mb-6">
                <svg class="w-16 h-16 lg:w-20 lg:h-20 text-slate-400" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                </svg>
            </div>
            <h3 class="text-xl lg:text-2xl font-bold text-slate-800 mb-2">No se encontraron productos</h3>
            <p class="text-sm lg:text-base text-slate-500">Intenta con otra búsqueda o categoría</p>
        </div>
    @else
        <!-- Estado Vacío -->
        <div class="flex flex-col items-center justify-center py-20 lg:py-32">
            ...
            <h3 class="text-xl lg:text-2xl font-bold text-slate-800 mb-2">No hay menú disponible</h3>
            <p class="text-sm lg:text-base text-slate-500">El menú del día aún no está listo. Vuelve pronto.</p>
        </div>
    @endif
    ...
    <div x-data="{
     ...
     "
        ...
        :class="tipo === 'exito' ? 'bg-linear-to-r from-green-500 to-green-600' :
            ...
            window.filtrarPorCategoria = function(categoria) {
                ...
                document.querySelectorAll('.boton-categoria').forEach(boton => {
                    ...
                    if (categoriaBoton === categoria) {
                        ...
                        boton.classList.add('bg-slate-100', 'hover:bg-slate-200', 'text-slate-600');
                        boton.classList.remove('bg-slate-700', 'text-white');
                    }
                });
