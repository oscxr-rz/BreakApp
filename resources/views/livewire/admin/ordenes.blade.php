<div class="h-screen bg-gray-50 flex flex-col overflow-hidden">
    ...
    @if (!empty($ordenes))
        ...
        <div class="hidden lg:flex flex-1 overflow-x-auto overflow-y-hidden px-6 py-4 snap-x snap-mandatory scroll-smooth"
            x-data="{ init() { this.handleScroll() }, handleScroll() { const container = this.$el; const cards = container.querySelectorAll('.orden-card'); const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.style.transform = 'scale(1)';
                                entry.target.style.opacity = '1'; } else { entry.target.style.transform = 'scale(0.85)';
                                entry.target.style.opacity = '0.5'; } }); }, { root: container, threshold: [0, 0.5, 1] });
                    cards.forEach(card => observer.observe(card)); } }" x-init="init()">
    ...
    wire:target="cambiarEstado({{ $orden['id_orden'] }})"
    ...
    <svg wire:loading.remove wire:target="cambiarEstado({{ $orden['id_orden'] }})" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <svg wire:loading wire:target="cambiarEstado({{ $orden['id_orden'] }})" class="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
    ...
    <span>{{ $orden['estado'] === 'PENDIENTE' ? 'Preparando' : 'Listo' }}</span>
        ...
        <div class="lg:hidden flex-1 overflow-y-auto px-4 py-4 snap-y snap-mandatory scroll-smooth"
            x-data="{ init() { this.handleScroll() }, handleScroll() { const container = this.$el; const cards = container.querySelectorAll('.orden-card-mobile'); const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.style.transform = 'scale(1)';
                                entry.target.style.opacity = '1'; } else { entry.target.style.transform = 'scale(0.9)';
                                entry.target.style.opacity = '0.6'; } }); }, { root: container, threshold: [0, 0.5, 1] });
                    cards.forEach(card => observer.observe(card)); } }" x-init="init()">
            <div class="space-y-4">
                @foreach ($ordenes as $orden)
