        ...
        <div class="px-4 py-5 flex items-center gap-3">
            ...
            <div class="relative group shrink-0">
                <div class="bg-white rounded-lg p-1.5 shadow-lg">
                    <img src="{{ asset('storage/logos/3.png') }}" alt="BreakApp Logo" class="w-10 h-10 object-contain">
                </div>
            </div>
        ...
        <nav class="flex-1 px-3 space-y-2 mt-6 relative">
            ...
            <div class="relative">
                <button id="ordenesDropdown"
                    ...
                    <svg id="ordenesChevron"
                        class="w-4 h-4 transition-transform duration-300 relative z-10 {{ request()->routeIs('admin.ordenes*') ? 'rotate-180' : '' }}"
                ...
                <div id="ordenesSubmenu"
                    class="overflow-hidden transition-all duration-300 {{ request()->routeIs('admin.ordenes*') ? 'max-h-96' : 'max-h-0' }}">
                    <div class="ml-6 mt-2 space-y-2 pb-2">
                        <a href="{{ route('admin.ordenes') }}"
                            class="flex items-center gap-2.5 px-3 py-2.5 text-sm transition-all duration-200 rounded-lg
                                   {{ request()->routeIs('admin.ordenes') && !request()->routeIs('admin.ordenes.*') ? 'text-blue-600 bg-white/90 font-medium shadow-sm' : 'text-blue-100 hover:bg-blue-700/30' }}">
                        ...
                        <a href="{{ route('admin.capturar.orden') }}"
                            class="flex items-center gap-2.5 px-3 py-2.5 text-sm transition-all duration-200 rounded-lg
                                   {{ request()->routeIs('admin.capturar.orden') ? 'text-blue-600 bg-white/90 font-medium shadow-sm' : 'text-blue-100 hover:bg-blue-700/30' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <a href="{{ route('admin.registrar.compra') }}"
                            class="flex items-center gap-2.5 px-3 py-2.5 text-sm transition-all duration-200 rounded-lg
                                   {{ request()->routeIs('admin.registrar.compra') ? 'text-blue-600 bg-white/90 font-medium shadow-sm' : 'text-blue-100 hover:bg-blue-700/30' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <!-- Tarjeta Local -->
            <div class="relative">
                <a href="{{ route('admin.tarjeta.local') }}"
                    class="nav-link relative flex items-center gap-2.5 px-3 py-3 transition-all duration-300 group z-20
              {{ request()->routeIs('admin.tarjeta.local') ? 'text-blue-600' : 'text-blue-100 hover:bg-blue-700/50 rounded-xl' }}">
                    <svg class="w-5 h-5 relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                    </svg>
                    <span class="font-medium text-sm relative z-10">Recargar Tarjeta</span>
                </a>
                @if (request()->routeIs('admin.tarjeta.local'))
                    <div
                        class="absolute -right-3 top-0 bottom-0 w-[calc(100%+12px)] bg-gray-50 rounded-tl-[40px] rounded-bl-[40px] pointer-events-none">
                    </div>
                @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (ordenesDropdown && ordenesSubmenu && ordenesChevron) {
                ordenesDropdown.addEventListener('click', (e) => {
                    ...
                    // Toggle submenu
                    const isOpen = ordenesSubmenu.classList.contains('max-h-96');

                    if (isOpen) {
                        ordenesSubmenu.classList.remove('max-h-96');
                        ordenesSubmenu.classList.add('max-h-0');
                        ordenesChevron.classList.remove('rotate-180');
                    } else {
                        ordenesSubmenu.classList.remove('max-h-0');
                        ordenesSubmenu.classList.add('max-h-96');
